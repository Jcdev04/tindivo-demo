...
export default function AuthOnboarding({ user, onClose, onComplete, onAuthMethod }) {
  ...
  const handleStartEmail = () => setStep('email');
  ...
  const stepIndex = ['method', 'email', 'phone', 'address'].indexOf(step);
  const showProgressChip = step === 'phone' || step === 'address';
  const progressNum = step === 'phone' ? 1 : 2;
  const showLaterHint = step === 'phone' || step === 'address';
      ...
      <div className="modal-sheet" onClick={e => e.stopPropagation()} style={{ maxHeight: '94%' }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
            <button onClick={step === 'email' ? () => setStep('method') : onClose} style={{
              ...
              {step === 'email' ? <Icon.Back/> : <Icon.Close/>}
            </button>
            {showLaterHint && (
              <div style={{
          ...
          {showProgressChip ? (
            ...
            <div style={{
              ...
              {step === 'method' ? 'Crear cuenta' : 'Con correo'}
            </div>
          )}
        </div>

        {/* Horizontal pager — 4 slides */}
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: 18,
                  background: refValid ? '#F97316' : 'rgba(249,115,22,0.32)',
                  color: '#fff',
                  fontFamily: 'inherit', fontSize: 15, fontWeight: 600,
                  letterSpacing: '-0.01em', cursor: 'pointer',
                  boxShadow: refValid ? '0 8px 22px -8px rgba(249,115,22,0.55)' : 'none',
                  transition: 'background 220ms ease, box-shadow 220ms ease',
                }}>
    ...
    <div style={{ padding: '20px 22px 0' }}>
      ...
      <div>
        ...
        <textarea
          ...
          placeholder="Ej. Casa color blanco con reja negra, frente a la bodega de don Carlos"
          maxLength={140}
          style={{
